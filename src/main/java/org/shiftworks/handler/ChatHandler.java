...
public class ChatHandler extends TextWebSocketHandler {
	 ...
	 * message) throws Exception 설명 : 웹 소켓 서버로 메세지를 전송했을 때 호출되는 메서드. 현재 웹 소켓 서버에 접속한
	 * Session 모두에게 메세지를 전달해야하므로 loop를 돌며 메세지를 전송한다.
	 * 
	...
	@Override
	protected void handleTextMessage(WebSocketSession webSocketSession, TextMessage chat) throws Exception {

		log.info("#ChattingHandler, handleTextMessage");
		
		// payload : 전송되는 데이터 자체 (JSON에서 payload는 data) 
...
//		}
	 ...
	 * Exception 설명 : 채팅을 위해 해당 페이지에 들어오면 ('/message/echo') 클라이언트가 연결된 후 클라이언트 세션을
	 * sessionList에 add 함.
	 * 
	 * @param session : '/message/echo'에 접속한 user
	 * @throws NullPointException 세션객체가 Null일 때 발생
	 ...
	 * status) throws Exception 설명 : 클라이언트와 연결이 끊어진 경우 (채팅방을 나간 경우) remove로 해당 세션을
	 * 제거한다.
	 * 
	 * @param session : '/message/echo'에 접속한 user
